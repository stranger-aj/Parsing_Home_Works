...
# для запросов через BeautifulSoup
...
while True:
    ...
    for item in items:
        ...
        # используем BeautifulSoup для извлечения описания книги
        link_into = session.get(url, headers=headers)
        soup = BeautifulSoup(link_into.text, 'html.parser')
        # много дочерних тегов с описанием
        description_parts = soup.find('div', id="product-about").findChildren()
        desc_union_text = [part.getText() for part in description_parts]
        description = ' '.join(' '.join(desc_union_text[1:]).split())
        book_dict = {
            'name': name,
            'author': author,
            'url': url,
